...
use common_lib::common_structs::{FormData, SESSION_USERNAME, SESSION_USER_ID};
...
pub async fn post_login(
    ...
    let correct_pw_hash = PasswordHash::new(stored_pw_hash)
        .expect("couldn't convert password hash stored in database to a proper Argon hash");

    let is_verified = Argon2::default().verify_password(form.password.as_bytes(), &correct_pw_hash);

    match is_verified {
        Ok(_verified) => {
            tracing::debug!("[post_login] is_verified: yes!");
            let _result =
                session.insert(SESSION_USER_ID, &login_result_unwrapped.user_id.to_string());
            let _result = session.insert(SESSION_USERNAME, &login_result_unwrapped.username);
            let message = format!(
                "Welcome, {}, ({})",
                &login_result_unwrapped.username, &login_result_unwrapped.user_id
            );
