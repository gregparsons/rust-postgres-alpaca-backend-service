...
pub async fn post_symbols(
    ...
    if let Ok(Some(_session_username)) = session.get::<String>(SESSION_USERNAME) {
        ...
        get_symbols_with_message(
            pool,
            hb,
            session,
            format!("{} [{}]", result_message, &symbol).as_str(),
        )
        .await
    } else {
        // not logged in
...
async fn get_symbols_with_message(
    pool: web::Data<PgPool>,
    hb: web::Data<Handlebars<'_>>,
    session: Session,
    message: &str,
...
async fn save_symbol_active(pool: &PgPool, symbol: &str, active: bool) -> Option<sqlx::Error> {
    ...
    .await
    {
        ...
        Err(e) => {
            tracing::debug!(
                "[save_symbol_active] error saving symbol and active field: {:?}",
                &e
            );
            Some(e)
        }
