...
async fn process_message(msg:DbMsg, pool: PgPool, tx_db: Sender<DbMsg>){
    ...
    match msg {
        ...
        DbMsg::AnalysisDailyProfitJsonOld { sender} => {
            match analysis_avg_daily_profit_old(&pool).await{
                Ok(vec_json) => {
...
async fn position_local_get(pool:PgPool)->Result<Vec<PositionLocal>, TradeWebError>{
    ...
    let result = sqlx::query_as!(PositionLocal, r#"
        select * from v_position_display
    "#).fetch_all(&pool).await;
...
async fn analysis_avg_daily_profit_old(pool:&PgPool) ->Result<Vec<ChartResult>, TradeWebError> {
