...
pub static MARKET_OPEN_EXTENDED:Lazy<NaiveTime> = Lazy::new(||{ NaiveTime::from_hms_opt(4, 0, 0).unwrap() }); // 4am Eastern
pub static MARKET_CLOSE_EXTENDED:Lazy<NaiveTime> = Lazy::new(||{ NaiveTime::from_hms_opt(20, 0, 0).unwrap() }); // 4pm
