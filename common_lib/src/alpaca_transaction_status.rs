...
impl AlpacaTransaction{
    ...
    pub async fn start_buy(symbol:&str, pool:&PgPool)->Result<(), TransactionError>{
        ...
        match sqlx::query!(r#"insert into alpaca_transaction_status(dtg, symbol, posn_shares) values(now()::timestamptz, $1, 0.0)"#, symbol.to_lowercase()).execute(pool).await{
            Ok(_)=>Ok(()),
    ...
    pub async fn decrement(symbol:&str, shares_to_decrement:BigDecimal, pool:&PgPool)->Result<(), TransactionError>{
        match sqlx::query!(
            r#"
                update alpaca_transaction_status set posn_shares = posn_shares - $1 where symbol=$2
            "#,
            shares_to_decrement,
            symbol.to_lowercase()
        ).execute(pool).await{
